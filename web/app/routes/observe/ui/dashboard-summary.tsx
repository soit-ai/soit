...
export function MetricTile({ item, onOpenRun }: { item: MetricCard; onOpenRun?: (url: string) => void }) {
  ...
  return (
    <Card
      ...
      aria-label={detailUrl ? t('observe.metric.openRunDetail', { target: item.label }) : undefined}
      onClick={openDetail}
    ...
    >
      <CardContent className="relative flex min-h-[94px] items-center gap-4 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {item.delta ? <div className="mt-2 text-xs text-muted-foreground">{t('observe.metric.deltaVsYesterday')} <span className={tone.delta}>{item.delta}</span></div> : null}
      ...
      <section className="grid overflow-hidden rounded-lg border border-border/80 bg-panel/95 shadow-[0_8px_22px_rgba(15,23,42,0.04)] lg:grid-cols-2 xl:grid-cols-4">
        <OverviewItem
          ...
          meta={dashboard.overview.workspace_health_status === 'healthy'
            ? t('observe.overview.healthy')
            : dashboard.overview.workspace_health_status === 'warning'
              ? t('observe.overview.warning')
              : t('observe.overview.needsAttention')}
        />
        <OverviewItem
          icon={AlertTriangle}
          ...
          meta={dashboard.overview.active_alert_count > 0
            ? t('observe.overview.needsAttention')
            : t('observe.overview.noActiveAlerts')}
        />
        <OverviewItem
          icon={Activity}
          iconClassName={infoIconSurface}
          label={t('observe.overview.sampling')}
        ...
        <OverviewItem
          icon={Clock3}
          iconClassName={infoIconSurface}
          label={t('observe.overview.lastRefresh')}
          value={t('observe.overview.justNow')}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {dashboard.recent_runs.slice(0, 6).map((run) => (
              <Button
                ...
                type="button"
                aria-label={t('observe.metric.openRunDetail', { target: run.run_id })}
                ...
                <div className="min-w-0">
                  ...
                  {run.observe_summary ? (
                    <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] font-medium text-muted-foreground">
                      <span>{t('observe.recentRuns.steps', { value: run.observe_summary.step_count })}</span>
                      <span>{t('observe.recentRuns.tools', { value: run.observe_summary.tool_call_count })}</span>
                      <span>{t('observe.recentRuns.citations', { value: run.observe_summary.citation_count })}</span>
      ...
      {dashboard.priority_alert ? (
        <div className={cn('flex min-h-9 flex-col gap-2 rounded-lg px-4 py-2 text-sm lg:flex-row lg:items-center lg:justify-between', dangerSurface)}>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            ...
            <span>{t('observe.priorityAlert.scope', { scope: dashboard.priority_alert.scope })}</span>
            <span>{t('observe.priorityAlert.affectedAgents', { value: dashboard.priority_alert.affected_agents })}</span>
            <span>{t('observe.priorityAlert.duration', { duration: dashboard.priority_alert.duration_label })}</span>
          </div>
          <Button
            variant="ghost"
            size="sm"
