...
import { getDocumentContent, downloadDocument } from '@/services/dataset-service'
...
export function DocumentPreviewDialog({
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      ...
      try {
        ...
        if (isBinary) {
          const response = await downloadDocument(datasetId, document.id)
          ...
          const text = await getDocumentContent(datasetId, document.id)
          if (active) {
            setContent(text)
          }
        }
      } catch (error) {
        toast.error(t('dataset.document.preview.fetchError'))
  ...
  const renderPreviewContent = () => {
    if (loading) {
      return <div>{t('dataset.document.preview.loading')}</div>
    }
    ...
    return <div>{t('dataset.document.preview.unavailable')}</div>
  }
