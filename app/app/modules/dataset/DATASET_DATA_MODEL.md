# Dataset 数据模块（长期稳定版 v1）

本文件定义 SOIT 知识库（Dataset）相关的**长期稳定数据模型**，目标是：
...
> 表命名按要求使用：`dataset`, `dataset_documents`, `dataset_chunks`, `dataset_indexs`
...
### 1.1 设计要点
- 一个 workspace 内，dataset 名称唯一
- dataset 维护默认的 chunk/retrieval 策略，但允许 index 覆盖
...
- 绑定与默认策略
  ...
  - `default_index_id`（可空，指向 dataset_indexs.id）
- 统计（冗余字段，便于列表展示）
...
## 2. dataset_documents（知识库文档）

### 2.1 设计要点
- 一个 dataset 内文档支持**多版本**（version）
...
- 解析/分块/索引元数据（引用对象存储）
  ...
  - `chunking_json`：本次版本采用的分块策略（可空，默认继承 dataset.chunking_json）
...
### 2.3 关键约束与索引
- UNIQUE：
  - `(tenant_id, workspace_id, dataset_id, doc_key, version)`
- 推荐索引：
  - `(tenant_id, workspace_id, dataset_id, is_latest)`
  - `(tenant_id, workspace_id, dataset_id, status)`
  - `(tenant_id, workspace_id, dataset_id, updated_at DESC)`
  - `(tenant_id, workspace_id, dataset_id, content_hash)`（可选，用于去重）

> `is_latest` 建议通过事务保证：同一 `(dataset_id, doc_key)` 只有一条 latest=true。

---

## 3. dataset_chunks（文档分块）
...
- 标识与隔离
  - `id` PK
  - `tenant_id`, `workspace_id` (NOT NULL)
  - `dataset_id`（冗余，便于查询；与 document 约束一致）
  - `document_id` (FK -> dataset_documents.id)
...
- 统计
  ...
  - `token_count`（可空）
- 向量映射与索引状态
  - `embedding_model_ref`（可空；默认继承 dataset/default 或 index）
...
- 索引：
  ...
  - `(tenant_id, workspace_id, dataset_id, index_status)`
  - `(tenant_id, workspace_id, dataset_id, updated_at DESC)`

---

## 4. dataset_indexs（索引配置与映射）

### 4.1 设计要点
- 一个 dataset 可以有多个 index（不同 embedding / 不同 provider / 不同检索配置）
...
- Provider/存储映射
  ...
  - `collection_name`（可空）
  - `partition_strategy`（可空：`tenant|workspace|dataset|none`）
  - `namespace`（可空：用于逻辑隔离）
- Embedding/检索配置
...
### 4.3 关键约束与索引
- UNIQUE：
  - `(tenant_id, workspace_id, dataset_id, name)`
- 索引：
  - `(tenant_id, workspace_id, dataset_id, status)`
  - `(tenant_id, workspace_id, dataset_id, is_primary)`
  - `(tenant_id, workspace_id, updated_at DESC)`

> `is_primary` 建议通过事务保证同一 dataset 只有一个 primary=true。

---

## 5. 推荐的对象存储 Key 规范（可选但强烈建议）
- 原始文件：`tenants/{tenant}/workspaces/{ws}/datasets/{ds}/raw/{file_id}`
...
- 过滤维度建议：`workspace_id + dataset_id + document_id + chunk_id`
...
- 再实现 index 构建：dataset_indexs.build_version 递增 + 统计回写
