"""Observability governance domain models."""
...
class ApprovalRequest(SQLModel, table=True):
    ...
    thread_id: Optional[str] = Field(default=None, nullable=True)
    agent_id: Optional[str] = Field(default=None, nullable=True)
    title: str = Field()
    policy_ref: Optional[str] = Field(default=None, nullable=True)
    status: str = Field(default="pending", index=True)
    details_json: dict[str, Any] = Field(default_factory=dict, sa_column=Column(JSON))
    requested_by: Optional[str] = Field(default=None, nullable=True)
