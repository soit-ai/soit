...
async def _seed_delivery(db, ctx):
...
@pytest.mark.asyncio
async def test_apprise_handler_resolves_secret_and_marks_delivery_sent(async_db, ctx):
    delivery = await _seed_delivery(async_db, ctx)
    secrets = SimpleNamespace(
        get_secret=AsyncMock(return_value="mailto://user:password@example.com")
    )
...
@pytest.mark.asyncio
async def test_apprise_handler_stops_after_five_attempts(async_db, ctx):
    delivery = await _seed_delivery(async_db, ctx)
