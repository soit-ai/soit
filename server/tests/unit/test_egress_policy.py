...
@pytest.mark.asyncio
async def test_egress_policy_allows_configured_domain(ctx, monkeypatch):
    ...
    await egress.check_egress_policy(
        ctx,
        "tool:http:test",
...
@pytest.mark.asyncio
async def test_egress_policy_denies_unlisted_domain(ctx, monkeypatch):
    monkeypatch.setattr(settings, "enable_egress_policy", True)
    monkeypatch.setattr(settings, "egress_allowlist", ["*.example.com"])
    monkeypatch.setattr(settings, "egress_blocklist", [])
    _reset_policy_cache(monkeypatch)
...
@pytest.mark.asyncio
async def test_egress_denial_details_do_not_expose_url_credentials(ctx, monkeypatch):
    monkeypatch.setattr(settings, "enable_egress_policy", True)
...
@pytest.mark.asyncio
async def test_egress_policy_disabled_allows(ctx, monkeypatch):
...
@pytest.mark.asyncio
async def test_egress_policy_uses_registered_scope_policy_provider(ctx, monkeypatch):
    monkeypatch.setattr(settings, "enable_egress_policy", True)
    monkeypatch.setattr(settings, "egress_allowlist", [])
    monkeypatch.setattr(settings, "egress_blocklist", [])
    _reset_policy_cache(monkeypatch)
...
@pytest.mark.asyncio
async def test_egress_policy_provider_failure_denies_request(ctx, monkeypatch):
    monkeypatch.setattr(settings, "enable_egress_policy", True)
    monkeypatch.setattr(settings, "egress_allowlist", ["*.example.com"])
    monkeypatch.setattr(settings, "egress_blocklist", [])
    _reset_policy_cache(monkeypatch)
...
@pytest.mark.asyncio
async def test_egress_policy_denies_private_ip_even_when_allowlisted(ctx, monkeypatch):
    monkeypatch.setattr(settings, "enable_egress_policy", True)
...
@pytest.mark.asyncio
async def test_egress_policy_cannot_be_bypassed_with_uppercase_scheme(ctx, monkeypatch):
    monkeypatch.setattr(settings, "enable_egress_policy", True)
    monkeypatch.setattr(settings, "egress_allowlist", ["allowed.example.com"])
    monkeypatch.setattr(settings, "egress_blocklist", [])
    _reset_policy_cache(monkeypatch)
