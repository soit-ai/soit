...
@pytest.mark.asyncio
async def test_identity_register_and_authenticate(async_db):
    """Register user and authenticate with password."""
    service = build_identity_service(db=async_db)
    ...
    user, tenant, token, workspace_id, refresh_token = await service.register_user(user_data, tenant_name="acme")
    ...
    role = await service.get_user_tenant_role(tenant.id, user.id)
    assert role == TENANT_ROLE_OWNER

    authed_user, authed_token, authed_workspace_id, authed_refresh = await service.authenticate_user(
        user_data.email,
        user_data.password,
    )
...
@pytest.mark.asyncio
async def test_identity_create_workspace_adds_owner_membership(async_db):
    ...
    user, tenant, _token, _workspace_id, _refresh = await service.register_user(user_data, tenant_name="acme-2")
    ...
    role = await service.get_user_workspace_role(workspace.id, user.id, ctx)
    assert role == WORKSPACE_ROLE_OWNER


@pytest.mark.asyncio
async def test_identity_update_workspace_quota_fields(async_db):
    ...
    user, tenant, _token, _workspace_id, _refresh = await service.register_user(user_data, tenant_name="acme-quota")
...
@pytest.mark.asyncio
async def test_identity_api_key_lifecycle(async_db):
    ...
    user, tenant, _token, _workspace_id, _refresh = await service.register_user(user_data, tenant_name="acme-3")
    ...
    keys = await service.list_api_keys(ctx, limit=10, offset=0)
    assert any(item.id == api_key.id for item in keys)

    rotated_key, rotated_raw = await service.rotate_api_key(api_key.id, ctx)
    ...
    old_key = await service.api_key_repo.get_by_id(api_key.id)
    assert old_key is not None
    assert old_key.status == "revoked"

    revoked = await service.revoke_api_key(rotated_key.id, ctx)
    assert revoked.status == "revoked"
    assert revoked.revoked_at is not None


@pytest.mark.asyncio
async def test_identity_rejects_legacy_roles(async_db):
    ...
    owner, tenant, _token, _workspace_id, _refresh = await service.register_user(owner_data, tenant_name="acme-legacy")
    ...
    member, _tenant, _token, _workspace_id, _refresh = await service.register_user(user_data, tenant_name=None)
