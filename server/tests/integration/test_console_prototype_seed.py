...
async def _scoped(async_db, model, summary):
    result = await async_db.exec(
        select(model).where(
            and_(
...
@pytest.mark.asyncio
async def test_seed_creates_the_objects_the_prototype_draws(async_db):
    ...
    names = {agent.name for agent in await _scoped(async_db, Agent, summary)}
    assert {"support-triage", "ops-copilot", "billing-audit"} <= names

    workflows = {item.name for item in await _scoped(async_db, Workflow, summary)}
    assert {"ticket-escalation", "docs-nightly-sync"} <= workflows

    knowledge = {item.name for item in await _scoped(async_db, Knowledge, summary)}
    assert {"product-docs", "billing-policies"} <= knowledge


@pytest.mark.asyncio
async def test_seed_honours_the_requested_volumes(async_db):
    from scripts.seed_console_prototype import seed_console_prototype

    summary = await seed_console_prototype(async_db, _args(runs=12, tasks=9))
...
@pytest.mark.asyncio
async def test_seed_is_idempotent_and_leaves_other_data_alone(async_db):
    from scripts.seed_console_prototype import seed_console_prototype

    first = await seed_console_prototype(async_db, _args())
    ...
    second = await seed_console_prototype(async_db, _args())
    third = await seed_console_prototype(async_db, _args(reset=False))
    ...
    assert await async_db.get(Thread, "thread_manual_not_seeded") is not None


@pytest.mark.asyncio
async def test_seeded_credentials_cannot_authenticate(async_db):
    from scripts.seed_console_prototype import seed_console_prototype
    ...
    keys = await _scoped(async_db, ApiKey, summary)
    assert len(keys) == 3
