...
class VirtualModelRepositoryPort(Protocol):
    """Workspace-scoped storage of virtual models."""

    async def get_by_id(self, id: str) -> Any: ...
    async def get_by_slug(self, slug: str) -> Any: ...
    async def list_all(self, limit: int = 200) -> list[Any]: ...
    async def create(self, model: Any) -> Any: ...
    async def update(self, model: Any) -> Any: ...
    async def delete(self, id: str) -> bool: ...


class ModelRefCatalogPort(Protocol):
    """Which concrete model refs name a model of the workspace."""
