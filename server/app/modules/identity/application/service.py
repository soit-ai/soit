...
class IdentityService:
    ...
    async def _ensure_workspace_membership(
        ...
        *,
        ...
        memberships = await membership_repo.get_by_user(user_id)
        if memberships:
        ...
        workspaces = await workspace_repo.list_by_tenant()
        if workspaces:
    ...
    async def register_user(
        ...
        *,
        ...
        existing_tenant = await self.tenant_repo.get_by_name(tenant_name)
        if existing_tenant:
        ...
        await self.tenant_membership_repo.create(membership)

        workspace_id, workspace_role = await self._ensure_workspace_membership(
            tenant_id=tenant.id,
            user_id=user.id,
            tenant_role=TENANT_ROLE_OWNER,
        )

        session, refresh_token = await self._issue_session(
            user_id=user.id,
            tenant_id=tenant.id,
    ...
    async def authenticate_user(
        ...
        *,
        ...
        memberships = await self.tenant_membership_repo.get_by_user(user.id)
        if not memberships:
        ...
        if await self.mfa_is_active(user.id):
    ...
    async def _issue_session(
        ...
        *,
        ...
        return await self.session_repo.create(session), refresh_token
    ...
    async def refresh_session(
        ...
        *,
        ...
        """
        session = await self.session_repo.get_by_refresh_hash(
        ...
        if user is None or not user.is_active:
            await self._end_session(session, revoked_by="system")
            raise UnauthorizedError("User account is inactive")
    ...
    async def list_sessions(self, ctx: RequestContext) -> list[UserSession]:
        """List the caller's own sessions, most recently active first."""
        return await self.session_repo.list_by_user(ctx.user_id)

    async def _end_session(self, session: UserSession, *, revoked_by: str) -> UserSession:
    ...
    async def revoke_session(self, ctx: RequestContext, session_id: str) -> UserSession:
        ...
        if session.status != "active":
            return session
        ended = await self._end_session(session, revoked_by=ctx.user_id)
        await self._log_session_audit(ctx, ended, operation="revoke")
        return ended
    ...
    async def revoke_all_sessions(
        ...
        *,
        ...
        ended = 0
        for session in await self.session_repo.list_by_user(ctx.user_id):
            if except_session_id and session.id == except_session_id:
                continue
            await self._end_session(session, revoked_by=ctx.user_id)
            ended += 1
        if ended:
            await self._log_session_audit(ctx, None, operation="revoke_all", count=ended)
        return ended

    async def _log_session_audit(
        self,
        ctx: RequestContext,
    ...
    async def get_deletion_request(self, ctx: RequestContext) -> AccountDeletionRequest | None:
        """The caller's pending closure request, if they have one."""
        return await self.deletion_repo.get_pending_for_user(ctx.user_id)
    ...
    async def request_account_deletion(
        ...
        existing = await self.deletion_repo.get_pending_for_user(ctx.user_id)
        if existing is not None:
            return existing

        if await self._is_last_tenant_owner(ctx.tenant_id, ctx.user_id):
        ...
        request = await self.deletion_repo.save(
            AccountDeletionRequest(
                user_id=ctx.user_id,
                tenant_id=ctx.tenant_id,
        ...
        await self._log_deletion_audit(ctx, request, operation="request")
        return request

    async def cancel_account_deletion(self, ctx: RequestContext) -> AccountDeletionRequest:
        """Withdraw a pending closure request."""
        request = await self.deletion_repo.get_pending_for_user(ctx.user_id)
        if request is None:
            raise NotFoundError("No closure request is pending")
        request.status = "cancelled"
        request.cancelled_at = utc_now()
        request.updated_at = utc_now()
        saved = await self.deletion_repo.save(request)
        await self._log_deletion_audit(ctx, saved, operation="cancel")
        return saved

    async def _is_last_tenant_owner(self, tenant_id: str, user_id: str) -> bool:
        memberships = await self.tenant_membership_repo.get_by_tenant(tenant_id)
        owners = [item for item in memberships if item.role == TENANT_ROLE_OWNER]
        return len(owners) == 1 and owners[0].user_id == user_id

    async def execute_account_deletion(self, request: AccountDeletionRequest) -> AccountDeletionRequest:
        ...
        for session in await self.session_repo.list_by_user(request.user_id):
            await self._end_session(session, revoked_by="account_closure")

        for key in await self.api_key_repo.list_by_user(request.user_id):
            if key.status == "active":
                key.status = "revoked"
                key.revoked_at = utc_now()
                await self.api_key_repo.update(key)

        enrolment = await self.mfa_repo.get_by_user(request.user_id)
        if enrolment is not None:
            await self.mfa_repo.delete(enrolment)
    ...
    async def execute_due_account_deletions(self, limit: int = 50) -> int:
        """Close every account whose pause has elapsed. Returns how many."""
        due = await self.deletion_repo.list_due(utc_now(), limit=limit)
        for request in due:
            await self.execute_account_deletion(request)
        return len(due)
    ...
    async def _issue_identity_token(self, user_id: str, purpose: str, ttl_minutes: int) -> str:
        """Mint a single-use link secret, retiring any earlier one."""
        await self.token_repo.supersede_pending(user_id, purpose)
    ...
    async def _consume_identity_token(self, token: str, purpose: str) -> User:
        """Spend a link secret and return whose it was."""
        record = await self.token_repo.get_by_hash(self._hash_token(token))
    ...
    async def request_password_reset(self, email: str, link_base: str) -> None:
        ...
        token = await self._issue_identity_token(user.id, "password_reset", PASSWORD_RESET_MINUTES)
    ...
    async def complete_password_reset(self, token: str, new_password: str) -> User:
        ...
        for session in await self.session_repo.list_by_user(user.id):
            await self._end_session(session, revoked_by="password_reset")
        return user
    ...
    async def request_email_verification(self, ctx: RequestContext, link_base: str) -> None:
        ...
        token = await self._issue_identity_token(
    ...
    async def confirm_email_verification(self, token: str) -> User:
        ...
        user = await self._consume_identity_token(token, "email_verification")
    ...
    async def invite_member(
        ...
        existing = await self.invitation_repo.get_pending_for_email(workspace_id, normalized)
        if existing is not None:
            ...
            raw = secrets.token_urlsafe(32)
            invitation = await self.invitation_repo.save(
                WorkspaceInvitation(
                    tenant_id=ctx.tenant_id,
                    workspace_id=workspace_id,
                    email=normalized,
    ...
    async def revoke_invitation(self, ctx: RequestContext, invitation_id: str) -> WorkspaceInvitation:
    ...
    async def accept_invitation(self, token: str, user_id: str) -> WorkspaceInvitation:
        """Redeem an invitation for an existing account."""
        invitation = await self.invitation_repo.get_by_hash(self._hash_token(token))
        ...
        if await self.tenant_membership_repo.get(invitation.tenant_id, user_id) is None:
            await self.tenant_membership_repo.create(
                TenantMembership(
                    tenant_id=invitation.tenant_id,
                    user_id=user_id,
        ...
        membership_repo = self.workspace_membership_repo_factory(ctx)
        if await membership_repo.get(invitation.workspace_id, user_id) is None:
            await membership_repo.create(
                WorkspaceMembership(
                    workspace_id=invitation.workspace_id,
                    user_id=user_id,
    ...
    async def get_mfa(self, user_id: str) -> UserMfa | None:
        """Return the user's enrolment, pending or active."""
        return await self.mfa_repo.get_by_user(user_id)

    async def mfa_is_active(self, user_id: str) -> bool:
        """Whether this user has a confirmed second factor."""
        enrolment = await self.mfa_repo.get_by_user(user_id)
    ...
    async def start_mfa_enrolment(
        ...
        """
        existing = await self.mfa_repo.get_by_user(ctx.user_id)
        ...
        if existing is not None:
            ...
            await self.mfa_repo.save(existing)
        else:
            await self.mfa_repo.save(
    ...
    async def confirm_mfa_enrolment(self, ctx: RequestContext, code: str) -> list[str]:
        ...
        await self.mfa_repo.save(enrolment)
        await self._log_mfa_audit(ctx, operation="enable")
        return codes

    async def disable_mfa(self, ctx: RequestContext, password: str) -> None:
        ...
        if enrolment is None:
            return
        await self.mfa_repo.delete(enrolment)
        await self._log_mfa_audit(ctx, operation="disable")

    async def regenerate_recovery_codes(self, ctx: RequestContext, code: str) -> list[str]:
        ...
        enrolment.updated_at = utc_now()
        await self.mfa_repo.save(enrolment)
        await self._log_mfa_audit(ctx, operation="recovery_codes_regenerated")
        return codes

    async def _verify_second_factor(self, user_id: str, code: str) -> bool:
    ...
    async def _log_mfa_audit(self, ctx: RequestContext, *, operation: str) -> None:
        self.db.add(
            AuditEvent(
                tenant_id=ctx.tenant_id,
                workspace_id=ctx.workspace_id,
    ...
    async def complete_mfa_login(
        ...
        *,
        ...
        if user is None or not user.is_active:
            raise UnauthorizedError("User account is inactive")
        if not await self._verify_second_factor(user_id, code):
            raise UnauthorizedError("That code is not valid")

        membership = next(
            (
                item
                for item in await self.tenant_membership_repo.get_by_user(user_id)
    ...
    async def list_saved_views(self, ctx: RequestContext, surface: str | None = None) -> list[SavedView]:
        """List the caller's kept filters, optionally for one screen."""
        return await self.saved_view_repo_factory(ctx).list(surface)

    async def create_saved_view(self, ctx: RequestContext, data: SavedViewCreate) -> SavedView:
        """Keep a filter under a name, replacing one of the same name."""
        repo = self.saved_view_repo_factory(ctx)
        existing = await repo.get_by_name(data.surface, data.name)
        if existing is not None:
        ...
        if view.is_default:
            await repo.clear_default(view.surface, except_id=view.id)
        return view
    ...
    async def update_saved_view(
        ...
        saved = await repo.save(view)
        if saved.is_default:
            await repo.clear_default(saved.surface, except_id=saved.id)
        return saved

    async def delete_saved_view(self, ctx: RequestContext, view_id: str) -> None:
    ...
    async def list_pins(self, ctx: RequestContext) -> list[PinnedObject]:
        """List the caller's pinned objects, most recent first."""
        return await self.pin_repo_factory(ctx).list()

    async def create_pin(self, ctx: RequestContext, data: PinCreate) -> PinnedObject:
        """Pin an object, or return the existing pin unchanged."""
        repo = self.pin_repo_factory(ctx)
        existing = await repo.get_by_target(data.object_type, data.object_id)
        if existing is not None:
            return existing
    ...
    async def delete_pin(self, ctx: RequestContext, pin_id: str) -> None:
        """Unpin an object."""
        repo = self.pin_repo_factory(ctx)
        pin = await repo.get(pin_id)
        if pin is None:
    ...
    async def create_workspace(
        ...
        existing = await workspace_repo.get_by_name(workspace_data.name)
        if existing:
    ...
    async def add_tenant_member(
        ...
        # Verify user exists
        user = await self.user_repo.get_by_id(membership_data.user_id)
        if not user:
            raise NotFoundError("User not found")
        ...
        existing = await self.tenant_membership_repo.get(tenant_id, membership_data.user_id)
        if existing:
        ...
        return await self.tenant_membership_repo.create(membership)
    ...
    async def add_workspace_member(
        ...
        tenant_membership = await self.tenant_membership_repo.get(ctx.tenant_id, membership_data.user_id)
        ...
        # Check if membership already exists
        membership_repo = self.workspace_membership_repo_factory(ctx)
        existing = await membership_repo.get(workspace_id, membership_data.user_id)
        if existing:
    ...
    async def list_workspaces(self, *, tenant_id: str, ctx: RequestContext) -> list[Workspace]:
    ...
    async def get_workspace(self, *, workspace_id: str, ctx: RequestContext) -> Workspace:
    ...
    async def get_user_workspace_role(
        ...
        membership_repo = self.workspace_membership_repo_factory(ctx)
        membership = await membership_repo.get(workspace_id, user_id)
    ...
    async def list_my_workspaces(self, ctx: RequestContext) -> list[tuple[Workspace, str]]:
        ...
        for membership in await membership_repo.get_by_user(ctx.user_id):
    ...
    async def update_workspace_member_role(
        ...
        if role not in WORKSPACE_ROLES:
            raise ValidationError("Invalid workspace role")
        repo = self.workspace_membership_repo_factory(ctx)
        membership = await repo.get(workspace_id, user_id)
        if not membership:
            raise NotFoundError("Workspace membership not found")
        membership.role = role
        return await repo.update(membership)
    ...
    async def remove_workspace_member(
        ...
        repo = self.workspace_membership_repo_factory(ctx)
        deleted = await repo.delete(workspace_id, user_id)
        if not deleted:
            raise NotFoundError("Workspace membership not found")
    ...
    async def get_user_tenant_role(
        ...
        membership = await self.tenant_membership_repo.get(tenant_id, user_id)
        return membership.role if membership else None
    ...
    async def rotate_api_key(
        ...
        old_key = await self.api_key_repo.get_by_id(key_id)
        if not old_key:
            raise NotFoundError(f"API key not found: {key_id}")
    ...
    async def create_resource_grant(
        ...
        existing = await grant_repo.get_by_resource_user(
            data.resource_type,
            data.resource_id,
            data.user_id,
        )
        ...
        if existing:
            ...
            existing.updated_at = utc_now()
            grant = await grant_repo.update(existing)
            operation = "update"
        else:
        ...
        await self._log_resource_grant_audit(
            ctx=ctx,
            resource_type=data.resource_type,
            resource_id=data.resource_id,
            user_id=data.user_id,
    ...
    async def revoke_resource_grant(
        ...
        await self._invalidate_permission_cache(ctx, resource_type, resource_id, user_id)
    ...
    async def list_resource_grants(
        ...
        *,
        ...
        if resource_type and resource_id:
            return await grant_repo.list_by_resource(resource_type, resource_id)
        return await grant_repo.list_in_scope(resource_type=resource_type, limit=limit)
    ...
    async def _invalidate_permission_cache(
        ...
        await cache.invalidate_permission(
            ctx=ctx,
            user_id=user_id,
            resource_type=resource_type,
            resource_id=resource_id,
    ...
    async def _log_resource_grant_audit(
        self,
        *,
        ctx: RequestContext,
        resource_type: str,
