"""Shared versioning and release orchestration."""

from __future__ import annotations

from typing import Any

from app.kernel.commons.errors import NotFoundError, ValidationError


class VersioningAdapter:
    """Base adapter for subject-specific version and release persistence.

    Persistence hooks are coroutines: the adapter runs on the request's
    `AsyncSession`, so every read or write it performs is awaited.
    """

    subject_kind = "subject"

    async def get_subject(self, subject_id: str) -> Any | None:
        raise NotImplementedError

    async def get_version(self, version_id: str) -> Any | None:
        raise NotImplementedError

    def version_matches_subject(self, version: Any, subject_id: str) -> bool:
        raise NotImplementedError

    async def next_version_number(self, subject_id: str) -> int:
        raise NotImplementedError

    async def create_version(
        self,
        subject_id: str,
        *,
        version_no: int,
        spec_schema: str,
        spec_json: dict[str, Any],
        based_on_version_id: str | None,
        metadata: dict[str, Any] | None,
    ) -> Any:
        raise NotImplementedError

    async def update_version(self, version: Any) -> Any:
        raise NotImplementedError

    async def update_head(self, subject: Any, version: Any) -> Any:
        raise NotImplementedError

    async def update_live(self, subject: Any, version: Any) -> Any:
        raise NotImplementedError

    async def create_release(
        self,
        subject: Any,
        version: Any,
        *,
        scope: str,
        status: str,
        notes: str | None,
        previous_live_version_id: str | None,
        rollback_of_publish_id: str | None = None,
    ) -> Any:
        raise NotImplementedError

    async def list_versions(self, subject_id: str, *, limit: int, offset: int) -> list[Any]:
        raise NotImplementedError

    async def list_releases(self, subject_id: str, *, limit: int, offset: int) -> list[Any]:
        raise NotImplementedError

    async def find_release_id_for_version(self, subject_id: str, version_id: str | None) -> str | None:
        return None

    async def validate_for_draft(
        self,
        subject: Any,
        *,
        spec_schema: str,
        spec_json: dict[str, Any],
        metadata: dict[str, Any] | None,
    ) -> None:
        return None

    async def validate_for_publish(self, subject: Any, version: Any) -> None:
        return None

    async def validate_for_rollback(self, subject: Any, version: Any) -> None:
        return None

    async def after_version_created(self, subject: Any, version: Any) -> None:
        return None

    async def after_publish(self, subject: Any, version: Any, release: Any) -> None:
        return None

    async def after_rollback(self, subject: Any, version: Any, release: Any) -> None:
        return None


class VersionControlService:
    """Shared orchestration for draft, publish, rollback, and version lookup."""

    def __init__(
        self,
        adapter: VersioningAdapter,
        *,
        ctx: Any | None = None,
        approval_checkpoint_gateway: Any | None = None,
    ) -> None:
        self.adapter = adapter
        self.ctx = ctx
        self.approval_checkpoint_gateway = approval_checkpoint_gateway

    def _require_publish_approval_if_needed(
        self,
        *,
        action: str,
        subject_id: str,
        version: Any,
        scope: str,
        notes: str | None,
    ) -> None:
        if self.ctx is None or self.approval_checkpoint_gateway is None:
            return

        request = {
            "action": action,
            "resource_type": self.adapter.subject_kind,
            "resource_ref": f"{self.adapter.subject_kind}:{subject_id}",
            "risk_level": "high",
            "run_id": None,
            "task_id": None,
            "thread_id": None,
            "agent_id": subject_id if self.adapter.subject_kind == "agent" else None,
            "title": f"Approve {action}: {self.adapter.subject_kind} {subject_id}",
            "details": {
                "subject_kind": self.adapter.subject_kind,
                "subject_id": subject_id,
                "version_id": getattr(version, "id", None),
                "version": getattr(version, "version", None),
                "scope": scope,
                "notes": notes,
            },
        }
        decision = self.approval_checkpoint_gateway.evaluate(self.ctx, request)
        if not bool(getattr(decision, "requires_approval", False)):
            return
        raise ValidationError(
            f"Approval required before {action}",
            details={
                "status": str(getattr(decision, "task_status", None) or "waiting_approval"),
                "reason": str(getattr(decision, "reason", "approval_required")),
                "policy_ref": getattr(decision, "policy_ref", None),
                "approval": dict(getattr(decision, "approval_payload", None) or {}),
                "request": request,
            },
        )

    async def _get_subject(self, subject_id: str) -> Any:
        subject = await self.adapter.get_subject(subject_id)
        if subject is None:
            raise NotFoundError(f"{self.adapter.subject_kind} not found: {subject_id}")
        return subject

    async def _get_version_for_subject(self, subject_id: str, version_id: str) -> tuple[Any, Any]:
        subject = await self._get_subject(subject_id)
        version = await self.adapter.get_version(version_id)
        if version is None or not self.adapter.version_matches_subject(version, subject_id):
            raise NotFoundError(f"{self.adapter.subject_kind} version not found: {version_id}")
        return subject, version

    async def create_draft(
        self,
        subject_id: str,
        *,
        spec_schema: str,
        spec_json: dict[str, Any],
        based_on_version_id: str | None = None,
        metadata: dict[str, Any] | None = None,
    ) -> Any:
        subject = await self._get_subject(subject_id)
        resolved_based_on = based_on_version_id
        if resolved_based_on is None:
            resolved_based_on = getattr(subject, "current_version_id", None)

        await self.adapter.validate_for_draft(
            subject,
            spec_schema=spec_schema,
            spec_json=spec_json,
            metadata=metadata,
        )
        version = await self.adapter.create_version(
            subject_id,
            version_no=await self.adapter.next_version_number(subject_id),
            spec_schema=spec_schema,
            spec_json=spec_json,
            based_on_version_id=resolved_based_on,
            metadata=metadata,
        )
        await self.adapter.after_version_created(subject, version)
        await self.adapter.update_head(subject, version)
        return version

    async def publish(
        self,
        subject_id: str,
        version_id: str,
        *,
        scope: str = "workspace",
        notes: str | None = None,
    ) -> Any:
        subject, version = await self._get_version_for_subject(subject_id, version_id)
        self._require_publish_approval_if_needed(
            action="publish",
            subject_id=subject_id,
            version=version,
            scope=scope,
            notes=notes,
        )
        await self.adapter.validate_for_publish(subject, version)
        version.status = "published"
        await self.adapter.update_version(version)
        release = await self.adapter.create_release(
            subject,
            version,
            scope=scope,
            status="published",
            notes=notes,
            previous_live_version_id=getattr(subject, "published_version_id", None),
        )
        subject = await self.adapter.update_live(subject, version)
        await self.adapter.after_publish(subject, version, release)
        return subject

    async def rollback(
        self,
        subject_id: str,
        version_id: str,
        *,
        scope: str = "workspace",
        notes: str | None = None,
        rollback_of_publish_id: str | None = None,
    ) -> Any:
        subject, version = await self._get_version_for_subject(subject_id, version_id)
        self._require_publish_approval_if_needed(
            action="rollback",
            subject_id=subject_id,
            version=version,
            scope=scope,
            notes=notes,
        )
        await self.adapter.validate_for_rollback(subject, version)
        version.status = "published"
        await self.adapter.update_version(version)
        previous_live_version_id = getattr(subject, "published_version_id", None)
        resolved_rollback_of_publish_id = rollback_of_publish_id
        if resolved_rollback_of_publish_id is None and previous_live_version_id:
            resolved_rollback_of_publish_id = await self.adapter.find_release_id_for_version(
                subject_id, previous_live_version_id
            )
        release = await self.adapter.create_release(
            subject,
            version,
            scope=scope,
            status="rolled_back",
            notes=notes,
            previous_live_version_id=previous_live_version_id,
            rollback_of_publish_id=resolved_rollback_of_publish_id,
        )
        subject = await self.adapter.update_live(subject, version)
        await self.adapter.after_rollback(subject, version, release)
        return subject

    async def get_head_version(self, subject_id: str) -> Any | None:
        subject = await self._get_subject(subject_id)
        version_id = getattr(subject, "current_version_id", None)
        if not version_id:
            return None
        version = await self.adapter.get_version(version_id)
        if version is None or not self.adapter.version_matches_subject(version, subject_id):
            return None
        return version

    async def get_live_version(self, subject_id: str) -> Any | None:
        subject = await self._get_subject(subject_id)
        version_id = getattr(subject, "published_version_id", None)
        if not version_id:
            return None
        version = await self.adapter.get_version(version_id)
        if version is None or not self.adapter.version_matches_subject(version, subject_id):
            return None
        return version

    async def list_versions(self, subject_id: str, *, limit: int, offset: int) -> list[Any]:
        await self._get_subject(subject_id)
        return await self.adapter.list_versions(subject_id, limit=limit, offset=offset)

    async def list_releases(self, subject_id: str, *, limit: int, offset: int) -> list[Any]:
        await self._get_subject(subject_id)
        return await self.adapter.list_releases(subject_id, limit=limit, offset=offset)
