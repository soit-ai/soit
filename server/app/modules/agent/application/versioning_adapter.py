...
class AgentVersioningAdapter(VersioningAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        sync_bindings: Callable[[Any, AgentVersion], Awaitable[None]],
    ) -> None:
    ...
    async def get_subject(self, subject_id: str) -> Any | None:
        return await self.agent_repo.get_by_id(subject_id)
    ...
    async def next_version_number(self, subject_id: str) -> int:
        return await self.agent_repo.next_version_number(subject_id)
    ...
    async def update_head(self, subject: Any, version: AgentVersion) -> Any:
        ...
        return await self.agent_repo.update(subject)

    async def update_live(self, subject: Any, version: AgentVersion) -> Any:
    ...
    async def list_versions(self, subject_id: str, *, limit: int, offset: int) -> list[Any]:
        return await self.version_repo.list_by_agent(subject_id, limit=limit, offset=offset)

    async def list_releases(self, subject_id: str, *, limit: int, offset: int) -> list[Any]:
        releases = await self.publish_repo.list_by_agent(subject_id)
    ...
    async def find_release_id_for_version(self, subject_id: str, version_id: str | None) -> str | None:
        if not version_id:
            return None
        for release in await self.publish_repo.list_by_agent(subject_id):
            if release.agent_version_id == version_id:
                return release.id
        return None

    async def after_version_created(self, subject: Any, version: AgentVersion) -> None:
        await self.sync_bindings(subject, version)
