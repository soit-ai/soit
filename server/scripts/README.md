...
- `seed_console_prototype.py`: idempotent seed of the objects the v13 console prototype draws (its agents, workflows, knowledge bases, plugins, models, secrets, threads, runs, tasks, approvals, team, API keys, credits and egress rules), so a fresh install can be reviewed against the design instead of showing empty lists. Independent of the MVP seeds: different id prefix and seed marker, so `--reset` on either leaves the other alone. `--runs` and `--tasks` set the volume (defaults match the prototype's reported 1,284 and 14). Seeded users and API keys are display-only -- their hashes are taken over random bytes, so nothing can authenticate with them, and secret values are never written at all.
- `seed_console_operations.py`: called by `seed_console_prototype.py`, not run on its own. Seeds the history those objects accumulate -- schedules, task events and checkpoints, decided approvals, notifications and endpoints, pins and saved views, resource grants, product feedback, policy revisions and egress audits, ingest tasks and chunks, draft reviews and releases, dead letters, workflow and knowledge-query runs, regression reports and run artifacts -- so every list, state and tile has rows. It seeds no `queued` ingest task, because the API's in-process ingest worker would claim it and fail it for lack of a stored file. To seed the local test account's own workspace: `python -m scripts.seed_console_prototype --email <user> --tenant-name <tenant> --workspace-name <workspace>`.
