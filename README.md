...
- MCP-friendly: any Model Context Protocol server resolves into the runtime tool registry without code changes. Transport is streamable HTTP. Protected servers are reached with OAuth 2.1 using authorization-server discovery (RFC 9728, RFC 8414 / OpenID Connect) and resource-bound tokens (RFC 8707), via the `client_credentials` grant — SOIT calls MCP servers on its own behalf, so the browser-based authorization-code flow is not implemented. The adapter that calls MCP servers targets the MCP SDK v1 line and does not yet speak the stateless 2026-07-28 revision; SOIT's own MCP endpoint serves it (see [SOIT as an MCP server](./docs/mcp.md))
